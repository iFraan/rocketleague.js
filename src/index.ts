...
import {
  SegmentOverviewStats,
  SegmentPlaylistStats,
  TrackerResponse,
} from "./types/tracker";
...
class API {
  ...
  static async fetchUser(platform: Platform, username: string) {
    ...
    instance._raw = (await fetchData(
      BASE_URL.replace("{PLATFORM}", platform).replace("{USERNAME}", username)
    )) as TrackerResponse;
    if (instance._raw.errors?.length > 0)
      throw new Error(instance._raw.errors[0].message);
    return instance;
  }
  ...
  get2v2(options: GenericOptions = {}) {
    ...
    const raw = options.raw ?? false;
    const data = this._raw.data.segments.find(
      (x) => x.metadata.name == "Ranked Doubles 2v2"
    );
  ...
  get3v3(options: GenericOptions = {}) {
    ...
    const raw = options.raw ?? false;
    const data = this._raw.data.segments.find(
      (x) => x.metadata.name == "Ranked Standard 3v3"
    );
    const stats = data.stats as SegmentPlaylistStats;
    if (raw) result._raw = data;
    result.rank = stats.tier.metadata.name;
  ...
  getData() {
    ...
    result.gamemodes = {};
    const playlists = this._raw.data.segments.filter(
      (x) => x.type === "playlist"
    );
    for (const playlist of playlists) {
      if (playlist) {
        ...
        result.gamemodes[playlist.metadata.name]["rank"] =
